...
PORT = 8081
REMOTE = "https://gentle-bjppjzad.peachworlds.com"

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

class ProxyHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        local_path = os.path.normpath(os.path.join(BASE_DIR, self.path.lstrip('/')))
        # Serve index.html for root/directory paths
        if local_path.startswith(BASE_DIR) and os.path.isdir(local_path):
            index_path = os.path.join(local_path, "index.html")
            if os.path.isfile(index_path):
                local_path = index_path
        if local_path.startswith(BASE_DIR) and os.path.exists(local_path) and os.path.isfile(local_path):
